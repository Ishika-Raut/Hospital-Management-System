...
public class DoctorDao
{
	...
	public boolean registerDoctor(Doctor d)
	{
		...
		try
		{
			String sql = "insert into doctor(fname, lname, dob, age, mobno, gender, qualification, specialist, email, password) values(?,?,?,?,?,?,?,?,?,?)";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, d.getFirstName());
			ps.setString(2, d.getLastName());
			ps.setString(3, d.getDob());
			ps.setString(4, d.getAge());
	...
	public List<Doctor> getAllDoctor()
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				d.setSpecialist(rs.getString(9));
	...
	public Doctor getDoctorByID(int id)
	{
		Doctor d = null;
		try
		{	System.out.println("getDocotrByID method");
	...
	public boolean updateDoctor(Doctor d)
	{
		...
		try
		{
			String sql = "update doctor set fname=?, lname=?, dob=?, age=?, mobno=?, gender=?, qualification=?, specialist=?, "
